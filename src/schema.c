...
/*
** The database schema for the ~/.vcs configuration database.
*/
const char zConfigSchema[] = 
@ -- This file contains the schema for the database that is kept in the
@ -- ~/.vcs file and that stores information about the users setup.
@ --
...
@
@ -------------------------------------------------------------------------
@ -- The BLOB and DELTA tables above hold the "global state" of a vcs
...
/*
** The schema for the locate vcs database file found at the root
