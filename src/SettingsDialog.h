...
#define FUEL_SETTING_VCS_PATH			"VCSPath"
...
struct Settings
{
	...
	Settings()
	{
		Mappings[FUEL_SETTING_VCS_PATH] = Setting();
		Mappings[FUEL_SETTING_COMMIT_MSG] = Setting();
		Mappings[FUEL_SETTING_FILE_DBLCLICK] = Setting(0); // Maps to FileDblClickAction
		Mappings[FUEL_SETTING_GDIFF_CMD] = Setting("", Setting::TYPE_VCS_GLOBAL);
		Mappings[FUEL_SETTING_GMERGE_CMD] = Setting("", Setting::TYPE_VCS_GLOBAL);
		Mappings[FUEL_SETTING_IGNORE_GLOB] = Setting("", Setting::TYPE_VCS_LOCAL);
		Mappings[FUEL_SETTING_CRNL_GLOB] = Setting("", Setting::TYPE_VCS_LOCAL);
		Mappings[FUEL_SETTING_REMOTE_URL] = Setting("off", Setting::TYPE_VCS_COMMAND);
	}
};
...
private slots:
	void on_btnSelectVCS_clicked();
	void on_buttonBox_accepted();
	void on_btnSelectVCSGDiff_clicked();
