...
** COMMAND: merge
**
** Usage: %vcs merge ?OPTIONS? VERSION
*/
void merge_cmd(void){
  ...
  */
  ...
  if( vid==0 ){
    vcs_fatal("nothing is checked out");
  }
  mid = name_to_typed_rid(g.argv[2], "ci");
  if( mid==0 || !is_a_version(mid) ){
    vcs_fatal("not a version: %s", g.argv[2]);
  }
  if( zPivot ){
    pid = name_to_typed_rid(zPivot, "ci");
    if( pid==0 || !is_a_version(pid) ){
      vcs_fatal("not a version: %s", zPivot);
    }
    if( pickFlag ){
      vcs_fatal("incompatible options: --cherrypick & --baseline");
    }
  }else if( pickFlag || backoutFlag ){
    pid = db_int(0, "SELECT pid FROM plink WHERE cid=%d AND isprim", mid);
    if( pid<=0 ){
      vcs_fatal("cannot find an ancestor for %s", g.argv[2]);
    }
  }else{
    ...
    pid = pivot_find();
    if( pid<=0 ){
      vcs_fatal("cannot find a common ancestor between the current "
  ...
  if( !is_a_version(pid) ){
    vcs_fatal("not a version: record #%d", pid);
  }
  if( detailFlag ){
  ...
  */
  ...
  if( debugFlag ){
    ...
    while( db_step(&q)==SQLITE_ROW ){
       vcs_print("%3d: ridv=%-4d ridp=%-4d ridm=%-4d chnged=%d isexe=%d "
                    " islinkv=%d islinkm=%d\n",
          db_column_int(&q, 0),
          db_column_int(&q, 5),
          db_column_int(&q, 6),
  ...
  */
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    vcs_warning("WARNING - no common ancestor: %s\n", zName);
    free(zName);
  ...
  */
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    if( file_wd_isfile_or_link(zFullName) ){
      vcs_print("ADDED %s (overwrites an unmanaged file)\n", zName);
      nOverwrite++;
    }else{
      vcs_print("ADDED %s\n", zName);
    }
  ...
  */
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    if( detailFlag ){
      vcs_print("MERGE %s  (pivot=%d v1=%d v2=%d)\n", 
  ...
  */
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    vcs_print("DELETE %s\n", zName);
    if( chnged ){
      vcs_warning("WARNING: local edits lost for %s\n", zName);
      nConflict++;
    }
    undo_save(zName);
    db_multi_exec(
  ...
  */
  if( !nochangeFlag ){
    if( nConflict ){
      vcs_print("WARNING: %d merge conflicts", nConflict);
    }
    if( nOverwrite ){
