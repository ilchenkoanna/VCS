...
/*
** COMMAND: test-tag
** %vcs test-tag (+|*|-)TAGNAME ARTIFACT-ID ?VALUE?
**
...
void testtag_cmd(void){
  ...
  switch( zTag[0] ){
    ...
    default:   
      vcs_fatal("tag should begin with '+', '*', or '-'");
      return;
  }
  rid = name_to_rid(g.argv[3]);
  if( rid==0 ){
    vcs_fatal("no such object: %s", g.argv[3]);
  }
...
void tag_cmd(void){
  ...
  if( strncmp(g.argv[2],"branch",n)==0 ){
    vcs_fatal("the \"vcs tag branch\" command is discontinued\n"
                 "Use the \"vcs branch new\" command instead.");
  }else
