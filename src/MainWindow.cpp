...
//------------------------------------------------------------------------------
// Open a vcs file or workspace path. If no checkout is detected offer to
// open the vcs file.
bool MainWindow::openWorkspace(const QString &path)
{
	...
	if(fi.isFile())
	{
		...
		QString checkout_file1 = wkspace + PATH_SEP
		QString checkout_file2 = wkspace + PATH_SEP

		if(!(QFileInfo(checkout_file1).exists() || QFileInfo(checkout_file2).exists()) )
		{
			if(QMessageBox::Yes !=DialogQuery(this, tr("Open VCS"), tr("A workspace does not exist in this folder.\nWould you like to create one here?")))
			{
			...
			if(!runvcs(QStringList() << "open" << QuotePath(repositoryFile)))
			{
...
{
	QString filter(tr("VCS Files (*.vcs _VCS_ .fslckout)"));

	QString path = QFileDialog::getOpenFileName(
		this,
		tr("Open VCS Repository"),
		QDir::currentPath(),
		filter,
		&filter);
...
{
	QString filter(tr("Repositories (*.vcs)"));

	// Get Repository file
	QString repo_path = QFileDialog::getSaveFileName(
		this,
		tr("New VCS Repository"),
		QDir::currentPath(),
		filter,
		&filter);
	...
	// Create repository
	if(!runvcs(QStringList() << "new" << QuotePath(repositoryFile)))
	{
...
void MainWindow::on_actionCloneRepository_triggered()
{
	...
	if(!url.password().isEmpty())
		url.setPassword("*****");
	QStringList logcmd = QStringList() << "vcs" << "clone" << url.toString() << repository;

	log("<b>&gt;"+logcmd.join(" ")+"</b><br>", true);

	// Clone Repo
	if(!runvcs(cmd, 0, RUNGLAGS_SILENT_INPUT))
	{
...
void MainWindow::scanWorkspace()
{
	...
	// Retrieve the status of files tracked by vcs
	QStringList res;
	if(!runVCS(QStringList() << "ls" << "-l", &res, RUNGLAGS_SILENT_ALL))
		return;
	...
	for(QStringList::iterator line_it=res.begin(); line_it!=res.end(); ++line_it)
	{
		...
		// Generate a RepoFile for all non-existant vcs files
	...
	stashMap.clear();
	res.clear();
	if(!runVCS(QStringList() << "stash" << "ls", &res, RUNGLAGS_SILENT_ALL))
		return;
...
MainWindow::RepoStatus MainWindow::getRepoStatus()
{
	...
	// We need to determine the reason why vcs has failed
	// so we delay processing of the exit_code
	if(!runvcsRaw(QStringList() << "info", &res, &exit_code, RUNGLAGS_SILENT_ALL))
...
//------------------------------------------------------------------------------
bool MainWindow::runvcs(const QStringList &args, QStringList *output, int runFlags)
{
	int exit_code = EXIT_FAILURE;
	if(!runvcsRaw(args, output, &exit_code, runFlags))
		return false;
...
//------------------------------------------------------------------------------
static QString ParsevcsQuery(QString line)
{
	// Extract question
...
//------------------------------------------------------------------------------
// Run vcs. Returns true if execution was successful regardless if vcs
// issued an error
bool MainWindow::runvcsRaw(const QStringList &args, QStringList *output, int *exitCode, int runFlags)
{
	...
	if(!silent_input)
	{
		...
		log("<b>&gt; vcs "+params+"</b><br>", true);
	}

	QString wkdir = getCurrentWorkspace();

	QString vcs = getvcsPath();

	if(detached)
		return QProcess::startDetached(vcs, args, wkdir);

	// Make StatusBar message
	QString status_msg = tr("Running vcs");
	if(args.length() > 0)
		status_msg = QString("vcs %0").arg(args[0].toCaseFolded());
	...
	process.start(vcs, args);
	if(!process.waitForStarted())
	{
		log(tr("Could not start vcs executable '") + vcs + "''\n");
		return false;
	}
	...
	while(true)
	{
		...
		#ifdef QT_DEBUG // Log vcs output in debug builds
		if(!input.isEmpty())
			qDebug() << input;
		#endif
		...
		{
			log(last_line);
			QString query = ParsevcsQuery(last_line);
			QMessageBox::StandardButton res = DialogQuery(this, "vcs", query, QMessageBox::YesToAll|QMessageBox::Yes|QMessageBox::No);
		...
		else if(have_query && have_yn_query)
		{
			log(last_line);
			QString query = ParsevcsQuery(last_line);
			QMessageBox::StandardButton res = DialogQuery(this, "vcs", query);
		...
		else if(have_query && have_an_query)
		{
			log(last_line);
			QString query = ParsevcsQuery(last_line);
			QMessageBox::StandardButton res = DialogQuery(this, "vcs", query, QMessageBox::YesToAll|QMessageBox::No);
...
//------------------------------------------------------------------------------
QString MainWindow::getvcsPath()
{
	// Use the user-specified vcs if available
	QString vcs_path = settings.Mappings[FUEL_SETTING_vcs_PATH].Value.toString();
	if(!vcs_path.isEmpty())
		return QDir::toNativeSeparators(vcs_path);

	QString vcs_exe = "vcs";
#ifdef Q_WS_WIN32
	vcs_exe += ".exe";
#endif
	// Use our vcs if available
	QString fuel_vcs = QDir::toNativeSeparators(QCoreApplication::applicationDirPath() + QDir::separator() + vcs_exe);

	if(QFile::exists(fuel_vcs))
		return fuel_vcs;

	// Otherwise assume there is a "vcs" executable in the path
	return vcs_exe;
}
//------------------------------------------------------------------------------
void MainWindow::loadSettings()
{
	if(qsettings->contains(FUEL_SETTING_vcs_PATH))
		settings.Mappings[FUEL_SETTING_vcs_PATH].Value = qsettings->value(FUEL_SETTING_vcs_PATH);
...
{
	// If we have a customize vcs path, save it
	QString vcs_path = settings.Mappings[FUEL_SETTING_vcs_PATH].Value.toString();
	qsettings->setValue(FUEL_SETTING_vcs_PATH, vcs_path);
...
bool MainWindow::diffFile(QString repoFile)
{
	...
	return runvcs(QStringList() << "gdiff" << QuotePath(repoFile), 0, RUNGLAGS_DETACHED);
}
...
bool MainWindow::startUI()
{
	if(uiRunning())
	{
		log(tr("vcs UI is already running\n"));
		return true;
	}

	vcsUI.setParent(this);
	vcsUI.setProcessChannelMode(QProcess::MergedChannels);
	vcsUI.setWorkingDirectory(getCurrentWorkspace());

	log("<b>&gt; vcs ui</b><br>", true);
	log(tr("Starting vcs UI. Please wait.\n"));
	QString vcs = getvcsPath();

	vcsUI.start(vcs, QStringList() << "ui");
	if(!vcsUI.waitForStarted() || vcsUI.state()!=QProcess::Running)
	{
		log(tr("%0 does not exist\n").arg(vcs));
		ui->actionvcsUI->setChecked(false);
		return false;
	}
	...
	ui->actionvcsUI->setChecked(true);
...
void MainWindow::stopUI()
{
	...
	{
#ifdef Q_WS_WIN
		vcsUI.kill(); // QT on windows cannot terminate console processes with QProcess::terminate
#else
		vcsUI.terminate();
#endif
	}
...
//------------------------------------------------------------------------------
void MainWindow::on_actionvcsUI_triggered()
{
	if(!uiRunning())
		startUI();
	else
...
{
	if(!uiRunning())
		ui->actionvcsUI->activate(QAction::Trigger);

	Q_ASSERT(uiRunning());

	QDesktopServices::openUrl(QUrl(getvcsHttpAddress()+"/timeline"));
}
...
void MainWindow::on_actionHistory_triggered()
{
	...
	for(QStringList::iterator it = selection.begin(); it!=selection.end(); ++it)
	{
		QDesktopServices::openUrl(QUrl(getvcsHttpAddress()+"/finfo?name="+*it));
	}
}
...
void MainWindow::on_actionCommit_triggered()
{
	...
	// Generate vcs parameters.
	QStringList params;
	params << "commit" << "--message-file" << QuotePath(comment_fname);

	// When a subset of files has been selected, explicitely specify each file.
	// Otherwise all files will be implicitly committed by vcs. This is necessary
	// when committing after a merge where vcs thinks that we are trying to do
...
void MainWindow::on_actionAdd_triggered()
{
	...
	// Do Add
	runvcs(QStringList() << "add" << QuotePaths(selection) );
...
void MainWindow::on_actionDelete_triggered()
{
	...
	{
		// Do Delete
		if(!runvcs(QStringList() << "delete" << QuotePaths(repo_files)))
			return;
	}
...
void MainWindow::on_actionRevert_triggered()
{
	...
	// Do Revert
	runvcs(QStringList() << "revert" << QuotePaths(modified_files) );
...
void MainWindow::on_actionRename_triggered()
{
	...
	// Do Rename
	runvcs(QStringList() << "mv" << QuotePath(fi_before.filePath()) << QuotePath(fi_after.filePath()) );
...
void MainWindow::on_actionUndo_triggered()
{
	...
	if(!runvcs(QStringList() << "undo" << "--explain", &res ))
		return;
...
void MainWindow::on_actionAbout_triggered()
{
	...
	if(runvcs(QStringList() << "version", &res, RUNGLAGS_SILENT_ALL) && res.length()==1)
	{
		int off = res[0].indexOf("version ");
		if(off!=-1)
			vcs_ver = tr("vcs version ")+res[0].mid(off) + "\n\n";
	}

	QMessageBox::about(this, tr("About Fuel..."),
					   QCoreApplication::applicationName() + " "+ QCoreApplication::applicationVersion() + " " +
						tr("a GUI frontend to the vcs SCM\n"
...
void MainWindow::on_actionUpdate_triggered()
{
	...
	if(!runvcs(QStringList() << "update" << "--nochange", &res, RUNGLAGS_SILENT_ALL))
		return;
	...
	// Do Update
	runvcs(QStringList() << "update" );

	refresh();
}

//------------------------------------------------------------------------------
void MainWindow::loadvcsSettings()
{
	// Also retrieve the vcs global settings
	QStringList out;
	if(!runvcs(QStringList() << "settings", &out, RUNGLAGS_SILENT_ALL))
		return;
	...
	for(Settings::mappings_t::iterator it=settings.Mappings.begin(); it!=settings.Mappings.end(); ++it)
	{
		...
		// Command types we issue directly on vcs
		if(type == Settings::Setting::TYPE_vcs_COMMAND)
		{
			// Retrieve existing url
			QStringList out;
			if(runvcs(QStringList() << name, &out, RUNGLAGS_SILENT_ALL) && out.length()==1)
				it.value().Value = out[0].trimmed();

			continue;
		}

		// Otherwise it must be a vcs setting
		if(!kv.contains(name))
			continue;
...
void MainWindow::on_actionSettings_triggered()
{
	...
	for(Settings::mappings_t::iterator it=settings.Mappings.begin(); it!=settings.Mappings.end(); ++it)
	{
		...
		if(type == Settings::Setting::TYPE_vcs_COMMAND)
		{
			...
			runvcs(QStringList() << "remote-url" << QuotePath(it.value().Value.toString()), 0, RUNGLAGS_SILENT_INPUT);
			continue;
		}

		Q_ASSERT(type == Settings::Setting::TYPE_vcs_GLOBAL || type == Settings::Setting::TYPE_vcs_LOCAL);
		...
		if(type == Settings::Setting::TYPE_vcs_GLOBAL)
			params << "-global";
...
//------------------------------------------------------------------------------
QString MainWindow::getvcsHttpAddress()
{
	return "http://127.0.0.1:"+vcsUIPort;
}
...
void MainWindow::on_actionRenameFolder_triggered()
{
	...
	for(int i=0; i<files_to_move.length(); ++i)
	{
		...
		if(!runvcs(QStringList() << "mv" <<  QuotePath(r->getFilePath()) << QuotePath(new_file_path)))
		{
...
void MainWindow::on_actionNewStash_triggered()
{
	...
	runvcs(QStringList() << "stash" << command << "-m" << stash_name << QuotePaths(stashed_files) );
	refresh();
}
...
void MainWindow::on_actionApplyStash_triggered()
{
	...
	for(QStringList::iterator it=stashes.begin(); it!=stashes.end(); ++it)
	{
		...
		if(!runvcs(QStringList() << "stash" << "apply" << *id_it))
		{
	...
	for(QStringList::iterator it=stashes.begin(); delete_stashes && it!=stashes.end(); ++it)
	{
		...
		if(!runvcs(QStringList() << "stash" << "drop" << *id_it))
		{
...
void MainWindow::on_actionDiffStash_triggered()
{
	...
	// Run diff
	runvcs(QStringList() << "stash" << "diff" << *id_it, 0);
}
