...
**
*/
static int vcs_stat(const char *zFilename, struct stat *buf, int isWd){
#if !defined(_WIN32)
...
#else
  int rc = 0;
  char *zMbcs = vcs_utf8_to_mbcs(zFilename);
  rc = stat(zMbcs, buf);
  vcs_mbcs_free(zMbcs);
  return rc;
#endif
...
static int getStat(const char *zFilename, int isWd){
  ...
  if( zFilename==0 ){
    ...
    if( vcs_stat(zFilename, &fileStat, isWd)!=0 ){
      fileStatValid = 0;
      rc = 1;
    }else{
      fileStatValid = 1;
...
void symlink_create(const char *zTargetFile, const char *zLinkFile){
  ...
  if( g.allowSymlinks ){
      ...
      if( zName[i]=='/' ){
        zName[i] = 0;
          if( file_mkdir(zName, 1) ){
            vcs_fatal_recursive("unable to create directory %s", zName);
            return;
          }
        zName[i] = '/';
      }
    ...
    if( symlink(zTargetFile, zName)!=0 ){
      vcs_fatal_recursive("unable to create symlink \"%s\"", zName);      
    }
  }else
#endif 
  {
...
void file_copy(const char *zFrom, const char *zTo){
  ...
  int got;
  char zBuf[8192];
  in = vcs_fopen(zFrom, "rb");
  if( in==0 ) vcs_fatal("cannot open \"%s\" for reading", zFrom);
  out = vcs_fopen(zTo, "wb");
  if( out==0 ) vcs_fatal("cannot open \"%s\" for writing", zTo);
...
#if !defined(_WIN32)
  struct stat buf;
  if( vcs_stat(zFilename, &buf, 1)!=0 || S_ISLNK(buf.st_mode) ) return 0;
  if( onoff ){
...
void file_delete(const char *zFilename){
  char *z = vcs_utf8_to_mbcs(zFilename);
  unlink(z);
  vcs_mbcs_free(z);
}
...
#if defined(_WIN32)
    int rc;
    char *zMbcs = vcs_utf8_to_mbcs(zName);
    rc = mkdir(zMbcs);
    vcs_mbcs_free(zMbcs);
    return rc;
#else
...
void file_getcwd(char *zBuf, int nBuf){
  ...
  if( getcwd(zPwd, sizeof(zPwd)-1)==0 ){
    vcs_fatal("cannot find the current working directory.");
  }
  zPwdUtf8 = vcs_mbcs_to_utf8(zPwd);
  nPwd = strlen(zPwdUtf8);
  if( nPwd > nBuf-1 ){
    vcs_fatal("pwd too big: max %d\n", nBuf-1);
  }
  for(i=0; zPwdUtf8[i]; i++) if( zPwdUtf8[i]=='\\' ) zPwdUtf8[i] = '/';
  memcpy(zBuf, zPwdUtf8, nPwd+1);
  vcs_mbcs_free(zPwdUtf8);
#else
  if( getcwd(zBuf, nBuf-1)==0 ){
    if( errno==ERANGE ){
      vcs_fatal("pwd too big: max %d\n", nBuf-1);
    }else{
      vcs_fatal("cannot find current working directory; %s",
                   strerror(errno));
    }
  }
#endif
...
void file_canonical_name(const char *zOrigName, Blob *pOut, int slash){
  if( file_is_absolute_path(zOrigName) ){
    ...
    */
    zOut = blob_str(pOut);
    if( vcs_isalpha(zOut[0]) && zOut[1]==':' ){
      zOut[0] = vcs_toupper(zOut[0]);
    }
#endif
  }else{
    char zPwd[2000];
    ...
    ** On Windows, normalize the drive letter to upper case.
    */
    if( vcs_isalpha(zPwd[0]) && zPwd[1]==':' ){
      zPwd[0] = vcs_toupper(zPwd[0]);
    }
#endif
    blob_zero(pOut);
...
#ifdef _WIN32
  if( vcs_isalpha(zIn[0]) && zIn[1]==':' ) zIn += 2;
#endif
  return zIn;
}
...
void file_relative_name(const char *zOrigName, Blob *pOut, int slash){
  ...
  if( zPath[0]=='/' ){
    ...
    i = 1;
#ifdef _WIN32
    while( zPath[i] && vcs_tolower(zPwd[i])==vcs_tolower(zPath[i]) ) i++;
#else
...
int file_tree_name(const char *zOrigName, Blob *pOut, int errFatal){
  ...
  if( nFull<=nLocalRoot || memcmp(zLocalRoot, zFull, nLocalRoot) ){
    ...
    blob_reset(&full);
    if( errFatal ){
      vcs_fatal("file outside of checkout tree: %s", zOrigName);
    }
    return 0;
  }
...
void file_tempname(int nBuf, char *zBuf){
  ...
  azDirs[1] = vcs_getenv("TEMP");
  azDirs[2] = vcs_getenv("TMP");
#endif
  ...
  */
  if( (strlen(zDir) + 17) >= (size_t)nBuf ){
    vcs_fatal("insufficient space for temporary filename");
  }

  do{
    if( cnt++>20 ) vcs_panic("cannot generate a temporary filename");
...
#if defined(_WIN32)
  vcs_mbcs_free((char *)azDirs[1]);
  vcs_mbcs_free((char *)azDirs[2]);
#endif
}
...
** Call vcs_mbcs_free() to deallocate any memory used to store the
** returned pointer when done.
*/
char *vcs_mbcs_to_utf8(const char *zMbcs){
#ifdef _WIN32
...
** translated text..  Call vcs_mbcs_free() to deallocate any memory
** used to store the returned pointer when done.
*/
char *vcs_utf8_to_mbcs(const char *zUtf8){
#ifdef _WIN32
...
*/
char *vcs_getenv(const char *zName){
  char *zValue = getenv(zName);
#ifdef _WIN32
  if( zValue ) zValue = vcs_mbcs_to_utf8(zValue);
#endif
  return zValue;
}
...
** used to store the returned pointer when done.
*/
char *vcs_utf8_to_console(const char *zUtf8){
#ifdef _WIN32
  int nChar, nByte;
...
/*
** Translate MBCS to UTF8.  Return a pointer.  Call vcs_mbcs_free()
** to deallocate any memory used to store the returned pointer when done.
*/
void vcs_mbcs_free(char *zOld){
#ifdef _WIN32
...
*/
FILE *vcs_fopen(const char *zName, const char *zMode){
  char *zMbcs = vcs_utf8_to_mbcs(zName);
