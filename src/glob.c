...
char *glob_expr(const char *zVal, const char *zGlobList){
  ...
  blob_zero(&expr);
  while( zGlobList[0] ){
    while( vcs_isspace(zGlobList[0]) || zGlobList[0]==',' ) zGlobList++;
    ...
    if( cTerm==',' ){
      while( i>0 && vcs_isspace(zGlobList[i-1]) ){ i--; }
    }
...
Glob *glob_create(const char *zPatternList){
  ...
  while( z[0] ){
    ...
    if( z[0]==0 ) break;
    p->azPattern = vcs_realloc(p->azPattern, (p->nPattern+1)*sizeof(char*) );
