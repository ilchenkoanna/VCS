...
static void diff_one_against_disk(
  ...
  if( !isLink != !file_wd_islink(zFrom) ){
    vcs_print("cannot compute difference between "
...
static void diff_all_against_disk(
  ...
  if( zFrom ){
    ...
    if( !is_a_version(rid) ){
      vcs_fatal("no such check-in: %s", zFrom);
    }
...
static void diff_one_two_versions(
  ...
  if( isLink1 != isLink2 ){
    diff_print_filenames(zName, zName, diffFlags);
    vcs_print("cannot compute difference "
...
static void diff_all_two_versions(
  ...
  while( pFromFile || pToFile ){
    ...
    if( pFromFile==0 ){
      ...
      cmp = vcs_strcmp(pFromFile->zName, pToFile->zName);
    }
    if( cmp<0 ){
      vcs_print("DELETED %s\n", pFromFile->zName);
      if( asNewFlag ){
      ...
      pToFile = manifest_file_next(pTo,0);
    }else if( vcs_strcmp(pFromFile->zUuid, pToFile->zUuid)==0 ){
      /* No changes */
      pFromFile = manifest_file_next(pFrom,0);
      pToFile = manifest_file_next(pTo,0);
    }else{
...
** COMMAND: gdiff
**
** Usage: %vcs diff|gdiff ?OPTIONS? ?FILE1? ?FILE2 ...?
**
*/
void diff_cmd(void){
  ...
  if( zTo==0 ){
    ...
    vcs_fatal("must use --from if --to is present");
  }else{
