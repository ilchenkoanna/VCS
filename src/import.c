...
** repository in the git-fast-import format as a new vcs
** repository.
*/
#include "config.h"
#include "import.h"
...
** Duplicate a string.
*/
char *vcs_strdup(const char *zOrig){
  char *z = 0;
  if( zOrig ){
    int n = strlen(zOrig);
    z = vcs_malloc( n+1 );
...
static void import_reset(int freeAll){
  int i;
  gg.xFinish = 0;
  vcs_free(gg.zTag); gg.zTag = 0;
  vcs_free(gg.zBranch); gg.zBranch = 0;
  vcs_free(gg.aData); gg.aData = 0;
  vcs_free(gg.zMark); gg.zMark = 0;
  vcs_free(gg.zDate); gg.zDate = 0;
  vcs_free(gg.zUser); gg.zUser = 0;
  vcs_free(gg.zComment); gg.zComment = 0;
  vcs_free(gg.zFrom); gg.zFrom = 0;
  vcs_free(gg.zFromMark); gg.zFromMark = 0;
  for(i=0; i<gg.nMerge; i++){
    vcs_free(gg.azMerge[i]); gg.azMerge[i] = 0;
  }
  gg.nMerge = 0;
  for(i=0; i<gg.nFile; i++){
    vcs_free(gg.aFile[i].zName);
    vcs_free(gg.aFile[i].zUuid);
    vcs_free(gg.aFile[i].zPrior);
  }
  memset(gg.aFile, 0, gg.nFile*sizeof(gg.aFile[0]));
  gg.nFile = 0;
  if( freeAll ){
    vcs_free(gg.zPrevBranch);
    vcs_free(gg.zPrevCheckin);
    vcs_free(gg.azMerge);
    vcs_free(gg.aFile);
...
static int fast_insert_content(Blob *pContent, const char *zMark, int saveUuid){
  ...
  if( saveUuid ){
    vcs_free(gg.zPrevCheckin);
    gg.zPrevCheckin = vcs_strdup(blob_str(&hash));
  }
  blob_reset(&hash);
  return rid;
}
...
static int mfile_cmp(const void *pLeft, const void *pRight){
  ...
  return vcs_strcmp(pA->zName, pB->zName);
}
...
static int string_cmp(const void *pLeft, const void *pRight){
  ...
  return vcs_strcmp(zLeft, zRight);
}
...
static void finish_commit(void){
  ...
  ** in sorted order and without any duplicates. Otherwise, vcs will not
  ** recognize the document as a valid manifest. */
  if( !gg.tagCommit && vcs_strcmp(zFromBranch, gg.zBranch)!=0 ){
  ...
  for(i=0; i<nTCard; i++){
    if( i==0 || vcs_strcmp(aTCard[i-1], aTCard[i]) ){
...
static ImportFile *import_add_file(void){
  ...
  if( gg.nFile>=gg.nFileAlloc ){
    ...
    gg.aFile = vcs_realloc(gg.aFile, gg.nFileAlloc*sizeof(gg.aFile[0]));
  }
...
static void import_prior_files(void){
  ...
  if( gg.zFrom==0 && gg.zPrevCheckin!=0 
   && vcs_strcmp(gg.zBranch, gg.zPrevBranch)==0
  ){
...
static void git_fast_import(FILE *pIn){
  ...
  while( fgets(zLine, sizeof(zLine), pIn) ){
    ...
    if( memcmp(zLine, "commit ", 7)==0 ){
      ...
      ** Git is changed to "trunk" which is the default name in vcs.
      ...
      */
      ...
      if( z[i+1]!=0 ) z += i+1;
      if( vcs_strcmp(z, "master")==0 ) z = "trunk";
      gg.zBranch = vcs_strdup(z);
      gg.fromLoaded = 0;
    }else
    ...
    if( memcmp(zLine, "data ", 5)==0 ){
      ...
      if( gg.nData ){
        int got;
        gg.aData = vcs_malloc( gg.nData+1 );
        got = fread(gg.aData, 1, gg.nData, pIn);
        if( got!=gg.nData ){
          vcs_fatal("short read: got %d of %d bytes", got, gg.nData);
        }
        gg.aData[got] = 0;
    ...
    if( memcmp(zLine, "tagger ", 7)==0 || memcmp(zLine, "committer ",10)==0 ){
      ...
      zLine[i] = 0;
      vcs_free(gg.zUser);
      gg.zUser = vcs_strdup(z);
      secSince1970 = 0;
      for(i=i+2; vcs_isdigit(zLine[i]); i++){
    ...
    if( memcmp(zLine, "from ", 5)==0 ){
      trim_newline(&zLine[5]);
      vcs_free(gg.zFromMark);
      gg.zFromMark = vcs_strdup(&zLine[5]);
      vcs_free(gg.zFrom);
    ...
    if( memcmp(zLine, "merge ", 6)==0 ){
      ...
      if( gg.nMerge>=gg.nMergeAlloc ){
        ...
        gg.azMerge = vcs_realloc(gg.azMerge, gg.nMergeAlloc*sizeof(char*));
      }
    ...
    if( memcmp(zLine, "M ", 2)==0 ){
      ...
      if( pFile==0 ){
        ...
        pFile->zName = vcs_strdup(zName);
      }
      pFile->isExe = (vcs_strcmp(zPerm, "100755")==0);
      pFile->isLink = (vcs_strcmp(zPerm, "120000")==0);      
      vcs_free(pFile->zUuid);
    ...
    if( memcmp(zLine, "D ", 2)==0 ){
      ...
      while( (pFile = import_find_file(zName, &i, gg.nFile))!=0 ){
        ...
        vcs_free(pFile->zName);
        vcs_free(pFile->zPrior);
        vcs_free(pFile->zUuid);
        *pFile = gg.aFile[--gg.nFile];
        i--;
      }
    ...
    if( memcmp(zLine, "C ", 2)==0 ){
      ...
      while( (pFile = import_find_file(zFrom, &i, mx))!=0 ){
        ...
        if( strlen(pFile->zName)>nFrom ){
          ...
          pNew->zName = vcs_strdup(pFile->zName);
        }
        pNew->isExe = pFile->isExe;
        pNew->isLink = pFile->isLink;
        pNew->zUuid = vcs_strdup(pFile->zUuid);
        pNew->isFrom = 0;
      }
    }else
    if( memcmp(zLine, "R ", 2)==0 ){
      ...
      vcs_fatal("cannot handle R records, use --full-tree");
    }else
...
malformed_line:
  trim_newline(zLine);
  vcs_fatal("bad fast-import line: [%s]", zLine);
  return;
}
...
** COMMAND: import
**
** Usage: %vcs import --git ?OPTIONS? NEW-REPOSITORY
**
** Read text generated by the git-fast-export command and use it to
** construct a new vcs repository named by the NEW-REPOSITORY
...
void git_import_cmd(void){
  ...
  ** Given any valid fast-import symbol, the corresponding vcs rid and
  ...
  */
  ...
  verify_cancel();
  db_end_transaction(0);
  vcs_print("Vacuuming..."); fflush(stdout);
