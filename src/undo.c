...
void undo_finish(void){
  if( undoActive ){
    if( undoNeedRollback ){
      vcs_print("\"vcs undo\" is available to undo changes"
...
void undo_rollback(void){
  ...
  undoNeedRollback = 0;
  undoActive = 0;
  vcs_print("Rolling back prior filesystem changes...\n");
...
** COMMAND: redo*
**
** Usage: %vcs undo ?OPTIONS? ?FILENAME...?
**    or: %vcs redo ?OPTIONS? ?FILENAME...?
**
*/
void undo_cmd(void){
  ...
  if( explainFlag ){
    if( undo_available==0 ){
      vcs_print("No undo or redo is available\n");
    }else{
      Stmt q;
      int nChng = 0;
      zCmd = undo_available==1 ? "undo" : "redo";
      vcs_print("A %s is available for the following command:\n\n"
                   "   %s %s\n\n",
      ...
      while( db_step(&q)==SQLITE_ROW ){
        if( nChng==0 ){
          vcs_print("The following file changes would occur if the "
      ...
      db_finalize(&q);
      if( nChng==0 ){
        vcs_print("No file changes would occur with this undo/redo.\n");
      }
    }
  }else{
